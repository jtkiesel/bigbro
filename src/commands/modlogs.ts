...
export class ModLogsCommand extends Command {
  public override async chatInputRun(interaction: ChatInputCommandInteraction) {
    ...
    const replyOptions = async (): Promise<BaseMessageOptions> => ({
      ...
      components: [
        ...
        await this.buttonActionRow(
          logPage,
          await this.getLogLength(selectedCategory, userLog),
        ),
      ],
    });
  ...
  private async getLogLength(selectedCategory: Logs, userLog: ModerationLog) {
    switch (selectedCategory) {
      case Logs.Warnings:
  ...
  private async userPage(userLog: ModerationLog, cache: CachedPages) {
  ...
  private async selectActionRow(selectedCategory: Logs) {
    return new ActionRowBuilder<StringSelectMenuBuilder>().setComponents(
      new StringSelectMenuBuilder().setCustomId(Logs.Category).addOptions([
        {
          label: "User",
  ...
  private async buttonActionRow(page: number, logsLength: number) {
    const isLastPage = page === logsLength - 1;
